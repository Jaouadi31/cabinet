...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'respond_message') {
        $message_id = $_POST['message_id'];
        $response_note = sanitize($_POST['response_note']);
        
        // Mark as replied and add internal note (we would need to add a response_note column to be complete)
...
<head>
    ...
    <title>Messages de Contact - Secrétaire</title>
...
<body>
    ...
    <div class="container" style="padding: 2rem 0;">
        <h1 style="color: var(--primary-green); margin-bottom: 2rem;">Messages de Contact</h1>
        ...
        <div style="margin: 2rem 0;">
            <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                ...
                <a href="../contact.php" class="btn btn-outline">📝 Voir Formulaire de Contact</a>
            </div>
        </div>
                                ...
                                <div style="flex: 1;">
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                                        ...
                                        <div style="display: flex; gap: 0.5rem;">
                                            ...
                                            <button onclick="respondToMessage(<?= $message['id'] ?>, '<?= htmlspecialchars($message['name']) ?>', '<?= htmlspecialchars($message['email']) ?>')" 
    ...
    <div id="responseModal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <div id="responseInfo" style="background: var(--very-light-green); padding: 1rem; border-radius: 8px; margin-bottom: 1rem;"></div>
            
            <form method="POST" id="responseForm">
                <input type="hidden" name="action" value="respond_message">
                <input type="hidden" name="message_id" id="response_message_id">
                
                <div class="form-group">
                    <label for="response_note">Note de réponse (interne):</label>
                    <textarea id="response_note" name="response_note" rows="4" 
                              placeholder="Notez ici comment vous avez répondu au client (par téléphone, email, etc.)..."></textarea>
                </div>
                
                <div style="background: #d1ecf1; padding: 1rem; border-radius: 8px; margin: 1rem 0;">
                    <h4 style="color: #0c5460;">💡 Conseils pour la réponse:</h4>
                    <ul style="margin: 0.5rem 0; padding-left: 1.5rem;">
                        <li>Appelez le client si un numéro de téléphone est fourni</li>
                        <li>Répondez par email pour confirmer la réception</li>
                        <li>Pour les demandes de RDV, orientez vers le système de prise de rendez-vous</li>
                        <li>Pour les urgences, transférez immédiatement vers le médecin de garde</li>
                    </ul>
                </div>
                
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">Marquer comme Répondu</button>
                    <button type="button" onclick="closeResponseModal()" class="btn btn-secondary">Annuler</button>
                </div>
            </form>
        </div>
    </div>
    ...
    function viewMessage(message) {
        const messageContent = `
            ...
            <div class="message-section">
                ...
                <p>
                    <span class="status-badge status-${message.status}">
                        ${message.status === 'new' ? '🆕 Nouveau' : message.status === 'read' ? '👁️ Lu' : '✅ Répondu'}
                    </span>
                </p>
            </div>
        `;
    ...
    function respondToMessage(messageId, clientName, clientEmail) {
        document.getElementById('response_message_id').value = messageId;
        
        const responseInfo = `
            <h4 style="color: var(--primary-green);">👤 Répondre à:</h4>
            <p><strong>Client:</strong> ${clientName}</p>
            <p><strong>Email:</strong> ${clientEmail}</p>
            <p><em>Marquez ce message comme répondu après avoir contacté le client.</em></p>
        `;
